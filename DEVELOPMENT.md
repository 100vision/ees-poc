...
产品路线图请参见 [ROADMAP.md](ROADMAP.md)。
...
**非 V1 范围**：无 Software Center、无软件仓库、无 AD 集成、无审批流。

### 架构变化

```
Demo ───────────────────────────── V1
ees-agent (本地 whitelist + 日志)    →  HTTPS ↔ ESC Server + PostgreSQL
右键菜单入口不变                     →  策略和日志集中管理
```
...
| Phase | 内容 | 周期 |
|-------|------|------|
| 1 | Server 基础设施 — Go 项目、PostgreSQL、HTTPS + Token 认证 | 3 周 |
| 2 | Agent 接入 Server — 策略远程化、日志上传、心跳、本地缓存 | 3 周 |
| 3 | Web 管理后台 — 仪表盘、白名单管理、提权日志查询 | 3 周 |
| 4 | 集成与部署 — 打包、文档、端到端验证 | 1 周 |
| 缓冲 | | 1 周 |
| **总计** | | **~11 周（3 个月）** |

详细任务拆分见 [ROADMAP.md](ROADMAP.md)。
